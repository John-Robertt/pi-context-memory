...
同一 URI、相同内容的并发同步共享一次进行中的请求；同 URI 的不同内容立即拒绝。每次同步重新读取 URI 内容，资源缺失时从当前已核对来源副本重建；多个实例竞争创建时，冲突方在同一调用期限内等待已提交内容并精确核对。成功由 OpenViking 接受写入和预期 URI 内容精确读回共同证明；`queue_status` 等处理诊断不参与生产判断。失败不删除来源，也不伪装成正常空结果。
