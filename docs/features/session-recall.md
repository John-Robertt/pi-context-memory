...
`read_source` 接收 entry ID，并在该 entry 仍属于当前 branch 时重新核对 Pi 权威条目，返回其规范化任务内容和完成状态；关联的完整工具结果已经通过 fullOutputRef 核验时，可以按有界切片继续读取该结果。thinking、运行元数据、被 Pi 排除的内容、FullOutputCandidate/本机路径与 control 正文不作为可召回事实。结果标明调用预算造成的截断和继续读取方式。
...
3. 本扩展不确认依赖该数据面的增强输出，并调用 `ctx.abort()`；
4. 诊断只说明本扩展责任事实，修复、继续或禁用由用户决定。
...
- 必要召回数据面失败后，本扩展不确认依赖它的增强输出；abort 与 transport 结果分别观测；
