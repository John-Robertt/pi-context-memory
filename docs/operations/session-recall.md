...
`.artifacts/openviking/runtime/state.json` 区分 `starting`、`ready`、`restarting`、`failed` 和 `stopped`，以 `active*` 表示实际模型、`target*` 表示应用目标，并保存冷启动配置诊断。Pi 持久状态只表达模型输入采用“Pi 原生”；`/memory-model` 按需显示文件、行列和字段错误。运行中配置错误保持旧实例；冷启动配置错误启动无 VLM 基础服务。扩展还核对启动器 PID、启动标识、锁和 ready 子进程，死亡进程状态不会继续显示为 ready。
