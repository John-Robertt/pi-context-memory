...
运行配置由项目基础配置、有效用户目标和匹配的内部 `MemoryRuntimeProfile` 编译为可重建的项目内 `0600` 文件。运行状态区分目标配置、profile、实际子进程、服务 readiness、模型能力证明和任务请求能力；这些生成文件不承担业务事实权威。
